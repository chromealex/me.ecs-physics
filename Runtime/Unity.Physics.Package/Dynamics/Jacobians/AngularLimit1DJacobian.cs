...
namespace ME.ECS.Essentials.Physics
{
    ...
    struct AngularLimit1DJacobian
    {
        ...
        public void Solve(ref MotionVelocity velocityA, ref MotionVelocity velocityB, tfloat timestep, tfloat invTimestep)
        {
            ...
            tfloat effectiveMass;
            {
                tfloat invEffectiveMass = math.csum(AxisInMotionA * AxisInMotionA * velocityA.InverseInertia +
        ...
        private tfloat CalculateError(quaternion motionBFromA)
        {
            ...
            tfloat angle = CalculateTwistAngle(jointBFromA, AxisIndex);

            // Angle is in [-2pi, 2pi].
            // For comparison against the limits, find k so that angle + 2k * pi is as close to [min, max] as possible.
            tfloat centerAngle = (MinAngle + MaxAngle) / 2.0f;
