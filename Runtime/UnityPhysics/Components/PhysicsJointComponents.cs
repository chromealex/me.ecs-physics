        ...
        )
        {
            primaryConeAndTwist = new PhysicsJoint
            {
                ...
                m_Constraints = new FixedList128Bytes<Constraint>
                {
                    ...
                    [k_RagdollPrimaryMaxConeIndex]    = Constraint.Cone(0, new FloatRange(0, math.min(math.abs(maxConeAngle), math.PI)), Constraint.DefaultSpringFrequency, Constraint.DefaultSpringDamping)
                }
            };
            ...
            perpendicularCone = new PhysicsJoint
            {
                ...
                {
                    Length = 2,
                    [k_RagdollPerpendicularRangeIndex]     = Constraint.Cone(0, math.clamp(angularPlaneRange.Sorted() + new float2(fv), new float2(0), new float2(math.PI)), Constraint.DefaultSpringFrequency, Constraint.DefaultSpringDamping),
