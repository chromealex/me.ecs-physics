...
namespace ME.ECS {
    ...
    internal unsafe struct RetainBlobAssetBatchPtr : IComponent {

        public BlobAssetBatch* BlobAssetBatchPtr;

    }

    internal unsafe struct RetainBlobAssetPtr : IComponent {
    ...
    internal unsafe struct BlobAssetOwner : IComponent {
        ...
        public void Release() {
            if (BlobAssetBatchPtr != null) BlobAssetBatch.Release(BlobAssetBatchPtr);
        }
    ...
    internal unsafe struct BlobAssetBatch {
        ...
        internal static BlobAssetBatch* CreateFromMemory(void* buffer, int expectedTotalDataSize) {
            ...
            if (batch->RefCount != 1) throw new System.ArgumentException("BlobAssetBatch.Refcount must be 1 on deserialized");

            var header = (BlobAssetHeader*)(batch + 1);
            for (int i = 0; i != batch->BlobAssetHeaderCount; i++) {
                header->ValidationPtr = header + 1;
                if (header->Allocator != Allocator.None) throw new System.ArgumentException("Blob Allocator should be Allocator.None");
                header = (BlobAssetHeader*)(((byte*)(header + 1)) + header->Length);
            }

            header--;

            if (header == (byte*)batch + batch->TotalDataSize) throw new System.ArgumentException("");
            #endif
        ...
        public static void Release(BlobAssetBatch* batch) {
            ...
            if (newRefCount <= 0) {
                ...
                #if ENABLE_UNITY_COLLECTIONS_CHECKS
                if (newRefCount < 0) throw new InvalidOperationException("BlobAssetBatch refcount is less than zero. It has been corrupted.");

                if (batch->TotalDataSize == 0) throw new InvalidOperationException("BlobAssetBatch has been corrupted. Likely it has already been unloaded or released.");
    ...
    internal unsafe struct BlobAssetReferenceData {
        ...
        void ValidateNonBurst() {
            ...
            if (validationPtr != m_Ptr) throw new InvalidOperationException("The BlobAssetReference is not valid. Likely it has already been unloaded or released.");
        }
        ...
        public void ValidateNotNull() {
            if (m_Ptr == null) throw new InvalidOperationException("The BlobAssetReference is null.");
    ...
    unsafe public struct BlobPtr<T> where T : struct {
        ...
        public ref T Value {
            get {
                #if ENABLE_UNITY_COLLECTIONS_CHECKS
                if (m_OffsetPtr == 0) throw new System.InvalidOperationException("The accessed BlobPtr hasn't been allocated.");
                #endif
    ...
    /*public static class BlobAssetSerializeExtensions {
        ...
        /// <summary>
        /// Serializes the blob asset data and writes the bytes to a <see cref="System.IO.BinaryWriter"/> instance.
        /// </summary>
        ...
        unsafe public static void Write<T>(this System.IO.BinaryWriter binaryWriter, BlobAssetReference<T> blob) where T : struct {
        ...
        /// <summary>
        /// Reads bytes from a <see cref="System.IO.BinaryReader"/> instance and deserializes them into a new blob asset.
        /// </summary>
        ...
        unsafe public static BlobAssetReference<T> Read<T>(this System.IO.BinaryReader binaryReader) where T : struct {
